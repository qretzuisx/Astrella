...
const AttributeSelector = ({ label, value, options, onSelect, type, shapes, colors, symbols, isLast }) => {
  ...
  return (
    <div className="relative flex-1 min-w-fit" ref={dropdownRef} style={{ zIndex: isOpen ? 100 : 10 }}>
      ...
      {isOpen && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 sm:mt-2 w-56 sm:w-64 bg-white/80 backdrop-blur-3xl rounded-[24px] sm:rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.2)] border border-white/40 overflow-hidden z-[200] animate-in zoom-in slide-in-from-top-2 duration-300">
