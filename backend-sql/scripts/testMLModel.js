/**
 * Test Script for ML Recommendation Model (SQL Version)
...
const testMLModel = async () => {
    ...
    try {
        ...
        const gownCount = await Gown.count({ where: { available: true, verified: true } });
        ...
        if (bookingCount === 0) {
            console.log('⚠️  Warning: No bookings in database. ML model needs data to learn!');
        ...
        if (userCount > 1) {
            ...
            if (userWithBookings && userWithBookings.user) {
                ...
                const similarRecs = await mlModel.getSimilarUserRecommendations(
                    userWithBookings.user.id.toString(),
                    3
                );
