/**
 * Machine Learning Recommendation Model (SQL Version)
...
class CollaborativeFilteringModel {
    ...
    async train() {
        ...
        try {
            ...
            for (const booking of bookings) {
                ...
                const gownId = booking.gownId.toString();
    ...
    async calculateGownSimilarity() {
        ...
        for (let i = 0; i < gowns.length; i++) {
            const gownA = gowns[i];
            const gownAId = gownA.id.toString();
            ...
            for (let j = 0; j < gowns.length; j++) {
                ...
                const gownB = gowns[j];
                const gownBId = gownB.id.toString();
    ...
    calculateAttributeSimilarity(gownA, gownB) {
        ...
        const avgPrice = (parseFloat(gownA.price) + parseFloat(gownB.price)) / 2;
...
class HybridRecommendationModel {
    ...
    async getRecommendations(userId, preferences, limit = 20) {
        ...
        const allGowns = await Gown.findAll({ 
            where: { 
                available: true, 
                verified: true 
            },
            include: [
                { 
                    model: User, 
                    as: 'owner',
                    attributes: ['id', 'name', 'shopName', 'shopDescription', 'shopAddress', 'shopCity']
                }
            ]
        });
